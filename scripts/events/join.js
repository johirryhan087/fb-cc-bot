...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.postimg.cc/GhRwKnfH/join-gif.gif';
...
.catch(error => {
    ...
    try {
      ...
      (typeof threadData.customJoin == "undefined") ? msg = `Member Name: {name}\n Member Number: {soThanhVien}th"\n─────────────────\n Group Rules: Promotion, Refer buisness and Selling is NOT ALLOWED\n─────────────────\n[ {time} - {thu} ]` : msg = threadData.customJoin;
